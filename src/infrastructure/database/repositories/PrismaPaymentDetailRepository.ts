...
export class PrismaPaymentDetailRepository implements IPaymentDetailRepository {
  async save(detail: PaymentDetail): Promise<PaymentDetail> {
    ...
    return PaymentDetail.fromPersistence(
      saved.id, saved.paymentId, saved.receiptNumber, saved.notes, saved.status, saved.createdAt
    );
  }

  async findByPaymentId(paymentId: string): Promise<PaymentDetail[]> {
    const found = await prisma.paymentDetail.findMany({
      where: { paymentId }
    });

    return found.map(d => PaymentDetail.fromPersistence(
      d.id, d.paymentId, d.receiptNumber, d.notes, d.status, d.createdAt
    ));
  }
  ...
  async findById(id: string): Promise<PaymentDetail | null> {
    ...
    return PaymentDetail.fromPersistence(
      found.id, found.paymentId, found.receiptNumber, found.notes, found.status, found.createdAt
    );
  }

  async findByReceiptNumber(receiptNumber: string): Promise<PaymentDetail | null> {
    const found = await prisma.paymentDetail.findUnique({
      where: { receiptNumber }
    });
    if (!found) return null;
    return PaymentDetail.fromPersistence(
  ...
  async update(detail: PaymentDetail): Promise<PaymentDetail> {
    ...
    return PaymentDetail.fromPersistence(
      updated.id, updated.paymentId, updated.receiptNumber, updated.notes, updated.status, updated.createdAt
    );
  }
}